...
##### DATABASE PART #####
try:
	conn= MySQLdb.connect(db= 'projet_V2', user= 'root', passwd= '', host='127.0.0.1')
...
for row in reader:
	...
	if url != "url":	
        ...
        lon= row[4]
        ip= row[5]
        requete="INSERT INTO geolocalisation (lat, lon, ip) VALUES('" +lat +"', '" +lon +"', '" +ip +"')"
        print (requete)
        cursor.execute (requete)
        urlid="SELECT id FROM geolocalisation"
        urlid_i="INSERT INTO requete(url_id) VALUE('" +urlid+ "')"
        print (urlid)
